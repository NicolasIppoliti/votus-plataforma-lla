...
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='workspace_private' and p.prosecdef),14::bigint,
  'all fourteen private workspace functions are security definer');
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='workspace_private' and p.proconfig @>
    array['search_path=pg_catalog, workspace_private, pg_temp']),14::bigint,
...
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  ...
  where n.nspname='workspace_private' and p.proname<>'authorization_facts_status'
    and acl.privilege_type='EXECUTE'),24::bigint,
...
select is((select count(*) from pg_policies where policyname like 'workspace\_%' escape '\'),34::bigint,
