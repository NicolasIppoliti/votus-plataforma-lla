...
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='workspace_private' and p.prosecdef),13::bigint,
  'all thirteen private workspace functions are security definer');
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='workspace_private' and p.proconfig @>
    array['search_path=pg_catalog, workspace_private, pg_temp']),13::bigint,
...
select is((select array_agg(r.rolname order by r.rolname) from pg_proc p
  ...
  array['workspace_admin_owner','workspace_admin_owner','workspace_admin_owner',
    ...
    'workspace_query_owner','workspace_review_ingest_owner','workspace_review_ingest_owner']::name[],
...
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  ...
  where n.nspname='workspace_private' and p.proname<>'authorization_facts_status'
    and acl.privilege_type='EXECUTE'),22::bigint,
...
select is((select count(*) from pg_policies where policyname like 'workspace\_%' escape '\'),32::bigint,
