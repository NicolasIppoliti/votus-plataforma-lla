...
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='workspace_private' and p.prosecdef),9::bigint,
  'all nine private workspace functions are security definer');
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='workspace_private' and p.proconfig @>
    array['search_path=pg_catalog, workspace_private, pg_temp']),9::bigint,
...
select is((select array_agg(r.rolname order by r.rolname) from pg_proc p
  ...
  array['workspace_admin_owner','workspace_admin_owner','workspace_admin_owner',
    ...
    'workspace_admin_owner','workspace_admin_owner','workspace_audit_owner']::name[],
