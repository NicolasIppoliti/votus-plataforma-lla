...
do $$ begin if to_regprocedure('workspace_private.platform_review_breakdown(integer,integer)') is not null or (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='workspace_private')<>18 or (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace cross join lateral aclexplode(coalesce(p.proacl,acldefault('f',p.proowner))) a where n.nspname='workspace_private' and p.proname<>'authorization_facts_status' and a.privilege_type='EXECUTE')<>30 or (select count(*) from pg_policies where policyname like 'workspace\_%' escape '\')<>41 then raise exception 'platform review breakdown rollback did not restore the exact prior function inventory'; end if; end $$;
...
do $$ declare facets_definition text; begin
  ...
  select lower(pg_get_functiondef(
   ...
   if position('authorized_names as materialized (' in facets_definition) = 0
      or position('selected_names as (' in facets_definition) = 0
      or position('district_metadata as (' in facets_definition) = 0
      or position('rr.source_kind=''official''' in facets_definition) = 0 then
     raise exception 'canonical authorized official facet metadata reapply did not restore its source-backed CTE plan';
   end if;
