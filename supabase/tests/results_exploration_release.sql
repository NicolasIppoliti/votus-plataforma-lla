...
do $$ begin
  ...
  if not has_function_privilege('authenticated', 'public.results_exploration_facets(uuid,uuid,text,text,text)', 'EXECUTE')
  ...
  if has_function_privilege('anon', 'public.results_exploration_facets(uuid,uuid,text,text,text)', 'EXECUTE')
