-- 0032
-- Keep the distrito RPC below the hosted timeout without changing its payload or safety
-- decisions. The core now classifies narrow per-source summaries and scans selected facts once
-- for all party, provenance, vote, row, and mesa aggregates instead of materializing and
-- repeatedly rescanning a wide classified relation.
begin;

alter function results_exploration_official(uuid,uuid,text,text,text,text,integer,text)
  rename to results_exploration_official_wrapper_0031;
revoke all on function results_exploration_official_wrapper_0031(uuid,uuid,text,text,text,text,integer,text)
  from public,anon,authenticated;
grant execute on function results_exploration_official_wrapper_0031(uuid,uuid,text,text,text,text,integer,text)
  to results_exploration_executor;

create function results_exploration_official_0032(
  p_election_id uuid,
  p_category_id uuid,
...
begin
  if p_election_id is null or p_category_id is null or p_distrito_code is null then
    return results_exploration_official_0029(
  ...
  return (
    with target_jurisdictions as materialized (
      ...
      select fact.archive_entry_id,fact.granularity,fact.list_id,fact.votes,
      ...
      select archive_entry_id,granularity,seccion_code,
      ...
      from raw_rows
      group by archive_entry_id,granularity,seccion_code
      ...
      from (
        ...
        where granularity='distrito' and effective_level='seccion'
        union all
      ...
      from (
        select effective_level,case
          when granularity='distrito' and effective_level='seccion' then 0::bigint
      ...
      select r.archive_entry_id,n.mapping_jurisdiction,r.list_id,
        count(*)::bigint rows,coalesce(sum(r.votes),0)::bigint votes,
        array_agg(distinct r.jurisdiction_id) jurisdiction_ids
      from raw_rows r
      join normalized_shapes n
        on jsonb_build_array(n.archive_entry_id,n.granularity,n.seccion_code)
      ...
      where s.exclusion_count=0
        ...
        and not (r.granularity='distrito' and n.effective_level='seccion')
        and not (r.granularity='mesa' and n.effective_level='mesa' and r.mesa_code is null)
      group by r.archive_entry_id,n.mapping_jurisdiction,r.list_id
    ), preaggregated as (
      ...
      select sum(rows)::bigint rows,sum(votes)::bigint votes,
        (select count(distinct jurisdiction_id)::bigint
          from selected_by_archive_party a
          cross join lateral unnest(a.jurisdiction_ids) jurisdiction_id) mesa_count
      from selected_by_archive_party
    ), archive_entries as (
      select jsonb_agg(archive_entry_id order by archive_entry_id) value
      from (select distinct archive_entry_id from selected_by_archive_party) entries
    )
    select case
...
revoke all on function results_exploration_official_0032(uuid,uuid,text,text,text,text,integer,text)
  from public,anon,authenticated;
grant execute on function results_exploration_official_0032(uuid,uuid,text,text,text,text,integer,text)
  to results_exploration_executor;
...
begin
  payload:=results_exploration_official_0032(
    p_election_id,p_category_id,p_distrito_code,p_seccion_code,p_circuito_code,
    p_establecimiento_code,p_mesa_code,p_requested_level
  );
...
alter function results_exploration_official_0032(uuid,uuid,text,text,text,text,integer,text)
