begin;
do $$ begin perform set_config('votus_drilldown_facets_down.workspace_query_owner',pg_has_role(current_user,'workspace_query_owner','SET')::text,true); if not pg_has_role(current_user,'workspace_query_owner','SET') then execute format('grant workspace_query_owner to %I',current_user); end if; end $$;
grant create on schema workspace_api to workspace_query_owner;
set role workspace_query_owner;
drop function workspace_api.official_facets(uuid,uuid,text,text,text,text);
...
revoke all on function workspace_api.official_facets() from public,anon,authenticated;
...
do $$ begin if to_regrole('service_role') is not null then revoke all on function workspace_api.official_facets() from service_role; end if; end $$;
do $$ begin if current_setting('votus_drilldown_facets_down.workspace_query_owner',true)='false' then execute format('revoke workspace_query_owner from %I',current_user); end if; end $$;
commit;
