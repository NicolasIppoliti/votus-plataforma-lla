...
create temporary table year_level_review_context_down_privileges(schema_create boolean not null) on commit drop;
insert into year_level_review_context_down_privileges values(has_schema_privilege('workspace_review_ingest_owner','workspace_private','CREATE'));
do $$ begin if not (select schema_create from year_level_review_context_down_privileges) then grant create on schema workspace_private to workspace_review_ingest_owner; end if; end $$;
...
begin
 select coalesce(sum(n),0),coalesce(jsonb_agg(jsonb_build_object('context_role',context_role,'source_kind',source_kind,'archive_availability',archive_availability,'election_year',election_year,'unknown_reason',unknown_reason,'count',n) order by context_role,source_kind,archive_availability,election_year,unknown_reason),'[]'::jsonb)
 into protected_count,categories from (select context_role,source_kind,archive_availability,election_year,unknown_reason,count(*) n from workspace_private.review_item_context where unknown_reason='source_archive_not_attributable' group by context_role,source_kind,archive_availability,election_year,unknown_reason) categorized;
 if protected_count>0 then raise exception 'year-level review context rollback refused: count=%, categories=%',protected_count,categories using errcode='23514'; end if;
end $$;
...
alter table workspace_private.review_item_context add constraint review_item_context_unknown_reason_check check (
 ...
 (unknown_reason is not null and unknown_reason in ('historical_archive_not_linked','historical_unclassified','writer_context_not_provided') and (context_role='unknown' or source_kind='unknown' or archive_availability='unknown'))
);
