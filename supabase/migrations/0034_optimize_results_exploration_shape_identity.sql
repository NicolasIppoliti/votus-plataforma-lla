-- 0034
-- Preserve district payload and refusal semantics while replacing the JSONB shape join with one
-- NULL-safe text-array identity computed per raw row and carried through normalization.
begin;

alter function results_exploration_official(uuid,uuid,text,text,text,text,integer,text)
  rename to results_exploration_official_wrapper_0033;
revoke all on function results_exploration_official_wrapper_0033(uuid,uuid,text,text,text,text,integer,text)
  from public,anon,authenticated;
grant execute on function results_exploration_official_wrapper_0033(uuid,uuid,text,text,text,text,integer,text)
  to results_exploration_executor;

create function results_exploration_official_0034(
  p_election_id uuid,
  p_category_id uuid,
...
revoke all on function results_exploration_official_0034(uuid,uuid,text,text,text,text,integer,text)
  from public,anon,authenticated;
grant execute on function results_exploration_official_0034(uuid,uuid,text,text,text,text,integer,text)
  to results_exploration_executor;
...
begin
  payload:=results_exploration_official_0034(
    p_election_id,p_category_id,p_distrito_code,p_seccion_code,p_circuito_code,
    p_establecimiento_code,p_mesa_code,p_requested_level
  );
...
alter function results_exploration_official_0034(uuid,uuid,text,text,text,text,integer,text)
