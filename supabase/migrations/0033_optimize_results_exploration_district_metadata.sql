-- 0033
-- Keep district metadata aggregation narrow without changing payload or refusal semantics.
-- Selected facts are materialized once, party groups carry only rows and votes, and metadata is
-- computed directly from the selected facts instead of retaining per-party jurisdiction arrays.
begin;

alter function results_exploration_official(uuid,uuid,text,text,text,text,integer,text)
  rename to results_exploration_official_wrapper_0032;
revoke all on function results_exploration_official_wrapper_0032(uuid,uuid,text,text,text,text,integer,text)
  from public,anon,authenticated;
grant execute on function results_exploration_official_wrapper_0032(uuid,uuid,text,text,text,text,integer,text)
  to results_exploration_executor;

create function results_exploration_official_0033(
  p_election_id uuid,
  p_category_id uuid,
...
revoke all on function results_exploration_official_0033(uuid,uuid,text,text,text,text,integer,text)
  from public,anon,authenticated;
grant execute on function results_exploration_official_0033(uuid,uuid,text,text,text,text,integer,text)
  to results_exploration_executor;
...
begin
  payload:=results_exploration_official_0033(
    p_election_id,p_category_id,p_distrito_code,p_seccion_code,p_circuito_code,
    p_establecimiento_code,p_mesa_code,p_requested_level
  );
...
alter function results_exploration_official_0033(uuid,uuid,text,text,text,text,integer,text)
