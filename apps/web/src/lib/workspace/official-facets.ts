...
function parseRows(payload: unknown): ParsedFacets {
  ...
  const rows = facets.map((raw: unknown) => {
  ...
  consistent(rows,(r)=>r.electionId,(r)=>[r.year,r.round]); consistent(rows,(r)=>r.categoryId,(r)=>r.categoryName); consistent(rows,(r)=>`${r.electionId}/${r.categoryId}/${r.distritoCode}`,(r)=>r.distrito,"facets.metadata.distrito_inconsistent"); consistent(rows,(r)=>`${r.electionId}/${r.categoryId}/${r.distritoCode}/${r.seccionCode}`,(r)=>r.seccion,"facets.metadata.seccion_inconsistent");
  noDuplicates(rows, (row) => [row.electionId,row.categoryId,row.distritoCode,row.seccionCode].join("\0"));
