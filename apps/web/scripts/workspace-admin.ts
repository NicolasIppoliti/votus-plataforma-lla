...
function databaseEnvironment(): NodeJS.ProcessEnv {
	...
	return { PATH: process.env.PATH ?? "", HOME: process.env.HOME ?? "", LANG: "C", NODE_ENV: "production", PGHOST: url.hostname, PGPORT: url.port || "5432", PGDATABASE: database, PGUSER: url.username, PGPASSWORD: decodeURIComponent(url.password), PGSSLMODE: "verify-full", PGSSLROOTCERT: ca };
}
