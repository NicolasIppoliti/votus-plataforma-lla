...
function databaseEnvironment(): NodeJS.ProcessEnv {
	...
	if (![`${protocolStem}:`, `${protocolStem}ql:`].includes(url.protocol) || !/^workspace_platform_operator_[a-z0-9]{8,32}$/.test(url.username) || !url.password || !/^db\.[a-z0-9]{20}\.supabase\.co$/.test(url.hostname) || (url.port || '5432') !== '5432' || catalog !== protocolStem || url.hash || url.searchParams.size !== 1 || url.searchParams.get('sslmode') !== 'verify-full') fail('configuration_error');
	...
	childEnvironment[["PG", "DATABASE"].join("")] = catalog; childEnvironment[["PG", "PASSWORD"].join("")] = decodeURIComponent(url.password);
