...
describe("migration release-gate integration", () => {
	const EXPECTED_MIGRATION_VERSIONS = Array.from({ length: 32 }, (_, index) =>
	...
	it("inspects the exact production migration and proof plan", async () => {
		...
		expect(plan.syntheticMigration).toEqual({
			version: "0033",
			fileName: "0033_e2e_service_role_grants.sql",
	...
	it("runs production proofs in order before installing synthetic 0033", () => {
	...
	it.each([
		...
		{
			defect: "extra",
			actual: [...EXPECTED_MIGRATION_VERSIONS, "0033"],
		},
	])("rejects a $defect migration inventory", ({ actual }) => {
		expect(() =>
			assertExactMigrationInventory(actual, EXPECTED_MIGRATION_VERSIONS),
		).toThrow("migration inventory must be exactly versions 0001 through 0032");
	});
	it("rejects a synthetic migration version collision", async () => {
		const plan = await inspectReleaseGatePlan();
		expect(() =>
			assertSyntheticMigrationDoesNotCollide(
				["0032_production.sql", "0033_production.sql"],
				plan.syntheticMigration,
			),
		).toThrow(
			"synthetic migration 0033 collides with production migration 0033_production.sql",
		);
	});
	...
	it("proves the exact results-exploration rollback through 0032 while leaving unrelated 0024 installed", () => {
		...
		expect(proof).toContain("32 as migration_inventory_count");
