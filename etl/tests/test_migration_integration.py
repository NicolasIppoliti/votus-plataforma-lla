...
SUPPORTED_MIGRATION_NUMBERS = frozenset(range(1, 36))
...
def _validated_migration_path(number: int, *, down: bool = False) -> Path:
    if type(number) is not int or number not in SUPPORTED_MIGRATION_NUMBERS:
        raise ValueError("migration number must be an integer from 1 through 35")
...
def test_migration_inventory_accepts_exact_history_through_0035() -> None:
    assert SUPPORTED_MIGRATION_NUMBERS == frozenset(range(1, 36))
    assert _available_migration_numbers() == list(range(1, 36))
    assert _validated_migration_path(35).name == "0035_reject_partial_pba_district_totals.sql"
    assert (
        _validated_migration_path(35, down=True).name
        == "0035_reject_partial_pba_district_totals.down.sql"
    )
    with pytest.raises(ValueError, match="1 through 35"):
        _validated_migration_path(36)
...
def test_0029_through_0035_forward_down_reapply_preserve_function_history_and_indexes() -> None:
    ...
    try:
        ...
        assert _available_migration_numbers(maximum=35) == list(range(1, 36))
