...
Historical planning STOP BEFORE APPLY is superseded by the human's documentation-only apply authorization; see [apply progress](apply-progress.md). Hosted RUN and delivery remain separately gated.
...
- [x] 1.2 Add the bounded observation sequence and stop rules in `docs/operational-verification.md`: observed/mismatch/unknown/refused-not-ready; deployment provenance, timestamp-only migrations, and distinct JWT, Auth-session, entitlement, grants, RLS, function, API, and route/status/cache claims. <!-- sdd-owner: implementation -->
